      ...
      fs.readFile(name, function(err, data) {
        ...
        request.post(options,function(error, response, body){
          ...
          if(error){                                //Case if request not properly made
            ...
            if(status == "success"){
              var name = 'doneRecog '+JSON.parse(body)["images"][0]["transaction"]["subject_id"];   //Sends recognized name with 'doneRecog' data to database when recognition is complete
